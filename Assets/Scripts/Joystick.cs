...
public class Joystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public RectTransform joystick_Rect; //조이스틱
    public Transform player; //캐릭위치
    Vector3 pos; //움직이는 방향
    float range; //조이스틱 이동범위
    bool touchFlag = false; //현재 드래그 중인가? 체크

    public float speedBuster = 1.2f;
    ...
    void Update()
    {
        if (GameManager.Instance.isFail == false)
        {
            if (touchFlag)
            {
                player.position += pos.normalized * (Time.deltaTime * speedBuster);
            }
        }
    }
